...
My Python solutions to the [Advent of Code](https://adventofcode.com) puzzles. They use the `aocd` library to automatically fetch puzzle inputs. Some solutions are command line scripts (the scripts folder), some are **JupyterLab** notebooks (the notebooks folder). Some are in both formats. The notebooks however, can also be run as command line scripts - no need to install Jupyter.

All puzzles run. My next task will be to test for correctness again (they all passed at the time I wrote them, but sometimes solutions can fail with different inputs). Lastly will be attempting to get all runtimes under 30 seconds (currently about 9 take too long).
