...
def run(inp1, inp2, is_real):
    ...
    def data_parse(inp):
        ...
        ic(len(lines))
        parsed = seq(lines).map(compose(str.split, map_int, list)).to_list()
        return parsed


    def valid_triangle(sides):
#        ics(sides)
#        ics(list(permutations(sides)))
        return all(a+b>c for a, b, c in permutations(sides))

    def process1(parsed):
        return seq(parsed).filter(valid_triangle).len()
    ...
    def process2(parsed):
#        chunks = chunks_of_n(parsed)
        chunks = [zip(*chunk) for chunk in chunks_of_n(parsed, 3)]
        return seq(chain(*chunks)).filter(valid_triangle).len()
