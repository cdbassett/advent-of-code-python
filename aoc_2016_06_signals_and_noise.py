...
def run(inp1, inp2, is_real):
    ...
    def process1(parsed):
        counters = [Counter(l) for l in zip(*parsed)]
        text = sjoin(counter.most_common(1)[0][0] for counter in counters)
        return text
    ...
    def process2(parsed):
        counters = [Counter(l) for l in zip(*parsed)]
        text = sjoin(counter.most_common()[-1][0] for counter in counters)
        return text
