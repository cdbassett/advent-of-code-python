...
# Transforms each element into a 2-tuple of that element
@extend()
def zip2(it):
...
@extend(final=True)
def to_string(it, sep=""):
    raise Exception("Use make_string()")
    return sep.join(it)


# similar to splitting a string
@extend()
def split(it, split_val=""):
    return (list(c) for match, c in itertools.groupby(it, lambda p: p == split_val) if not match)

# similar to splitting a string
@extend()
def splitby(it, is_delim_func):
    return (list(c) for match, c in itertools.groupby(it, is_delim_func) if not match)

# translation of itertools.groupby
# with the exception that groups are converted to lists
@extend()
def groupby(it, key=None):
    return ((match, list(c)) for match, c in itertools.groupby(it, key))

# translation of itertools.groupby
# with the exception that groups are converted to lists
# but throwing away the key
@extend()
def groupby_no_key(it, key=None):
    return (list(c) for match, c in itertools.groupby(it, key))
...
# new sequence consisting of old sequence plus another iterable
@extend()
def concat(it, iterable):
    return itertools.chain(it, iterable)

# make into a repeatable sequence (based on list of input sequence)
# creates new sequence based on list of sequence so far
@extend(aslist=True, final=True)
def build(it):
...
def skip(iterator, n=None):
    raise Exception("Use drop")
    it_ut.consume(iterator, n)
    return iterator


# return iterator of values for keys in iterator
@extend()
def lookup(iterator, dct):
    return map(dct.get, iterator)


@extend()
def lookup_default(iterator, dct, default=None):
    return map(func_utils.rpartial(dct.get, default), iterator)

@extend()
def level2_map(iterator, func):
    return (map(func, el) for el in iterator)

    #return map(list, (map(func, el) for el in iterator))

@extend()
def level2_map_tuple(iterator, func):
    return (tuple(map(func, el)) for el in iterator)
...
__FROM_ITERATION_UTILITIES__ = ""

# can use seq.sliding instead
#@extend()
#def successive(it, times=2):
#    return it_ut.successive(it, times=times)
...
@extend()
def tail_n(it, n):
    return it_ut.tail(it, n)

    # Pad the `iterable` with `fillvalue` in front and behind.
@extend()
def pad(iterable, fillvalue=None, nlead=0, ntail=0):
    return it_ut.pad(iterable, fillvalue, nlead, ntail)

@extend()
def pad_front(iterable, fillvalue=None, nlead=1):
    return it_ut.pad(iterable, fillvalue, nlead=nlead)

@extend()
def pad_back(iterable, fillvalue=None, ntail=1):
    return it_ut.pad(iterable, fillvalue, ntail=ntail)
...
def getitem(iterable, idx=None, start=None, stop=None, step=None):
    return it_ut.getitem(iterable, idx, start, stop, step)

    # Insert one `element` into `iterable`.
@extend()
def insert(iterable, element, idx, unpack=False):
    return it_ut.insert(iterable, element, idx, unpack)
...
@extend()
def starfilter(iterable, pred):
    return it_ut.starfilter(pred, iterable)

    # Return the first value in the iterable and expects it only contains one element.
@extend(final=True)
def one(iterable):
    return it_ut.one(iterable)
...
def replicate(iterable, times):
    ...
    return it_ut.replicate(iterable, times)

    # Alternately yield an item from the iterable and e
    # does not yield e as last item
@extend()
def intersperse(iterable, e):
    return it_ut.intersperse(iterable, e)
...
def minmax(iterable, /, key=None, default=None):
    it_ut.minmax(iterable, key=None, default=None)
...
class iteration_utilities.unique_justseen(iterable, key=None)
...
iteration_utilities.all_equal(iterable, /)
...
def combinations(it, r):
    return itertools.combinations(it, r)

# r-length tuples, in sorted order, all possible orderings
@extend()
def permutations(it, r=None):
    return itertools.permutations(it, r)
...
#@extend()
#def repeat(it, times=None):
#    return itertools.repeat(it)

__BUILT_INS__ = ""

# ======================================================================
# already built in, just copied from source here for easy lookup by name
# ======================================================================
def built_ins():
    add = None
    WRITE_MODE = None
...
#    aggregate(func)/aggregate(seed, func)
#      aggregate(seed, func, result_map)                    Aggregate using func starting with seed or first element of list then apply result_map to the result                      action
...
#    dict(default=None) / to_dict(default=None)             Converts a sequence of (Key, Value) pairs to a dictionary. If default is not None, it must be
#                                                             a value or zero argument callable which will be used to create a collections.defaultdict                                action
...
#    sliding(size, step=1)                                  Groups elements in fixed size blocks by passing a sliding window over them.
