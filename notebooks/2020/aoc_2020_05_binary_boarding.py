...
# %%
def convert_pass_to_id(bp):
    b = bp.replace("F", "0").replace("B", "1").replace("L", "0").replace("R", "1")
    return int(b, 2)

def process(parsed):
    ics(parsed)
    return seq(parsed).map(convert_pass_to_id).max()
...
# %%
def process2(parsed):
    ids = seq(parsed).map(convert_pass_to_id).list()

    for a, b in pairwise(sorted(ids)):
        if b == a+2:
            return b-1
