...
# %%
def process(parsed, preamble=25):
    #ics(parsed)
    
    for n, window in seq(parsed).sliding(preamble+1).enumerate():
        #ics(n, window)
        *prev, num = window
        
        if num not in (sum(ns) for ns in combinations(window, 2)):
            return num
...
# %%
def part1(inp, preamble=25):
    parsed = parse_data(inp)
    result = process(parsed, preamble)
    print_result(result)
