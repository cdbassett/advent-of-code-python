...
# %%
def parse_data(inp):
    return ParsedCharArray(inp.strip().split("\n"))
...
def process(parsed, slope = (3, 1)):
    ...
    for px, py in zip(count(0, sx), range(0, h, sy)):
        #ics(px, py, parsed[px % w, py])
        if parsed[px % w, py] == "#":
            cnt += 1
...
# %%
def process2(parsed):
    return math.prod(process(parsed, slope) for slope in [(1,1), (3,1), (5,1), (7,1), (1,2)])
