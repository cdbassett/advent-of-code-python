...
# %%
def parse_data(inp):
    return CharTable(list(map(list, inp.strip().split("\n"))))
...
def process(parsed):
    def removable(x, y):
        return seq(parsed.iter_neigh8(x, y, is_roll)).size() < 4
        
    #ics(parsed)
    cnt = seq(parsed.iter_all(is_roll)).starcount(removable)
    return cnt
...
def process2(parsed):
    ...
    while True:
        rm = seq(parsed.iter_all(is_roll)).starfilter(removable).list()
