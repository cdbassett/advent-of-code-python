...
def match3(s):
    return 3 in Counter(s).values()

def process(parsed):
    ics(parsed)
    return seq(parsed).count(match2) * seq(parsed).count(match3)
...
def common_chars(s1, s2):
    return sjoin(c1 for c1, c2 in zip(s1, s2) if c1 == c2)

def process2(parsed):
    ics(parsed)

    for id1, id2 in combinations(parsed, 2):
